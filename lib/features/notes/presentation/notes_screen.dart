      ...
      listener: (context, state) {
        if (state.actionStatus == NotesStatus.success) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(const SnackBar(content: Text('Note queue updated.')));
        }
        if (state.actionStatus == NotesStatus.failure) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(state.error ?? 'Note action failed.')),
          );
        }
      },
          ...
          children: [
                  ...
                  children: [
                    ...
                    TextField(
                      ...
                      decoration: const InputDecoration(
                        hintText:
                            'Write a study observation or flight lesson note',
                      ),
                    ),
                    const SizedBox(height: 12),
                    AppButton(
            ...
            if (state.notes.isEmpty)
              const EmptyState(
                ...
                message: 'Saved notes persist locally and wait for sync.',
              )
            else
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            OutlinedButton.icon(
              onPressed: canSync
                  ? () => context.read<NotesCubit>().syncNote(note)
                  : null,
              icon: const Icon(Icons.sync_rounded),
              label: Text(
                note.syncStatus == SyncStatus.failed
                    ? 'Retry Sync'
                    : 'Sync Now',
              ),
            ),
